...
const { vdom, cleanWhitespace, hash } = require('./utils');
...
exports.bundle = (opts) => {
  ...
  traversy(root, exts, (path) => {
    ...
    const verbose$ = vdom(template);
    const $ = vdom(cleanWhitespace(template));
    const whitespaceTagsSelectors = 'pre, code, textarea';
    const originalWhitespaceTags = verbose$(whitespaceTagsSelectors);
