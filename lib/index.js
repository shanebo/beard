...
const { cleanWhitespace, escape, hash } = require('./utils');
...
class Beard {
  constructor(opts = {}) {
    ...
    this.opts.customTags = merge(defaultTags, this.opts.customTags || {});
    this.configureCustomTags(this.opts.customTags);

    if (this.opts.shortcuts) {
      this.configureShortcuts(this.opts.shortcuts, this.opts.customTags);
    }
  }

  configureCustomTags(customTags) {
    const tags = Object.keys(customTags).join('|');
    exps.customTag = new RegExp(`^(${tags})\\\s+([^,]+)(?:,\\\s*([\\\s\\\S]*))?$`);

    const contentTags = Object.keys(customTags).filter((key) => customTags[key].content).join('|');
  ...
  configureShortcuts(shortcuts, customTags) {
    const tags = Object.keys(shortcuts).join('|');
    exps.shortcut = new RegExp(`^@(${tags})(?:(?:\\\s+)([\\\s\\\S]+))?$`);

    const contentTags = Object.keys(shortcuts).filter((key) => customTags[shortcuts[key].tag].content).join('|');
    if (contentTags.length) {
  ...
  compiled(path, parentPath = '') {
    ...
    if (!this.fns[path]) this.fns[path] = compile(str, path, this.opts.root);
  ...
  shortcut(name, parentPath, data) {
    ...
    return this.customTag(tag, parentPath, path, data);
  }
